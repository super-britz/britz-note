...
递归之容易爆栈，是因为一直新增调用栈，ECS 放不下。

- 尾递归：因为需要上一个栈的信息，所以没有出栈，而然还是需要创建新的调用栈，容易爆栈。

- 尾调用：上一个执行栈结束了，不需要上一个栈信息，可以出栈了，所以不容易爆栈。

- 尾递归优化：因为尾调用每次还是要创建一个新的栈，是不是浪费呢，复用一个栈帧是不是更好呢？ 但是浏览器不支持。（因为出错了浏览器找不到 ）JS的元编程可以开启尾递归优化。